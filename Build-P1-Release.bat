...
if errorlevel 1 (
  echo.
  echo ERROR: Focused TerrainCore regression failed. The release was not built.
  echo.
  if /i not "%~1"=="nopause" pause
  exit /b 1
)
...
if errorlevel 1 (
  echo.
  echo ERROR: The packaged TerrainCore regression failed. The release was not assembled.
  echo.
  if /i not "%~1"=="nopause" pause
  exit /b 1
)
